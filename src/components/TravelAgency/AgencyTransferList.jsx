...
export default function AgencyTransferList() {
    ...
    const [searchBy, setSearchBy] = useState('id'); // 'id' or 'hotel'
    ...
    const fetchTransfers = async () => {
        try {
            const token = localStorage.getItem('access_token');
            const response = await axios.get(`${BASE_URL}api/travel-agency-transfer/`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
